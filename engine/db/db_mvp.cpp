...
DBMVP::DBMVP(meta::DatabaseSchema& database_schema, int64_t init_table_scale, bool is_leader) {
  ...
  for (int i = 0; i < database_schema.tables_.size(); ++i) {
    auto table = std::make_shared<TableMVP>(database_schema.tables_[i], db_catalog_path_, init_table_scale_, is_leader_);
