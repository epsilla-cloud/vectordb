...
#ifdef __AVX2__
#include "db/index/transpose-avx2-inl.hpp"
#endif
...
VECTORDB_PRAGMA_IMPRECISE_FUNCTION_BEGIN
float fvec_inner_product(const float* x, const float* y, size_t d) {
    float res = 0.F;
    VECTORDB_PRAGMA_IMPRECISE_LOOP
    for (size_t i = 0; i != d; ++i) {
        res += x[i] * y[i];
    }
    return res;
}
VECTORDB_PRAGMA_IMPRECISE_FUNCTION_END
...
void compute_PQ_dis_tables_dsub2(
        ...
        float* dis_tables) {
    size_t M = d / 2;
    // VECTORDB_THROW_IF_NOT(ksub % 8 == 0);
