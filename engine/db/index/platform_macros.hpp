...
#define VECTORDB_DEPRECATED(msg) __attribute__((deprecated(msg)))
#else
#define VECTORDB_DEPRECATED(msg)
#endif // GCC or Clang

// Localized enablement of imprecise floating point operations
// You need to use all 3 macros to cover all compilers.
#if defined(_MSC_VER)
#define VECTORDB_PRAGMA_IMPRECISE_LOOP
#define VECTORDB_PRAGMA_IMPRECISE_FUNCTION_BEGIN \
    __pragma(float_control(precise, off, push))
#define VECTORDB_PRAGMA_IMPRECISE_FUNCTION_END __pragma(float_control(pop))
...
#define VECTORDB_PRAGMA_IMPRECISE_FUNCTION_END _Pragma("float_control(pop)")
#else
#define VECTORDB_PRAGMA_IMPRECISE_FUNCTION_BEGIN
#define VECTORDB_PRAGMA_IMPRECISE_FUNCTION_END
#endif
