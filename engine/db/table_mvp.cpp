...
namespace vectordb
{
  namespace engine
  {
          ...
          table_segment_(nullptr)
    {
      ...
      for (int i = 0; i < table_schema_.fields_.size(); ++i)
      {
            ...
            table_schema_.fields_[i].field_type_ == meta::FieldType::VECTOR_DOUBLE)
        {
          ...
          for (int executorIdx = 0; executorIdx < NumExecutorPerField; executorIdx++)
          {
        ...
        bool with_distance)
    {
      ...
      //   execution::RAIIVecSearchExecutor(executor_pool_[field_offset], executor_pool_[field_offset]->acquire);
      ...
      executor.exec_->Search(query_data, K, table_segment_->record_number_, result_num);
      auto status = Project(query_fields, result_num, executor.exec_->search_result_, result, with_distance, executor.exec_->distance_);
      if (!status.ok())
      {
        return status;
      }
